    ...
    async execute(interaction) {
        let run2 = async function () {
            ...
            const log =
                ...
                ) || newstate.guild.channels.cache.get('891799846095319110')
            // 2 5 10 20 30 40 50
            var role2 = interaction.guild.roles.cache.find(
                (role) => role.name === 'Mitglieder (Vc)'
            )
            var role5 = interaction.guild.roles.cache.find(
                (role) => role.name === 'BronzeNewbie (Vc)'
            )
            var role10 = interaction.guild.roles.cache.find(
                (role) => role.name === 'SilverBaby (Vc)'
            )
            var role20 = interaction.guild.roles.cache.find(
                (role) => role.name === 'GoldenDiva (Vc)'
            )
            var role30 = interaction.guild.roles.cache.find(
                (role) => role.name === 'Diamond (Vc)'
            )
            var role40 = interaction.guild.roles.cache.find(
                (role) => role.name === 'Master (Vc)'
            )
            var role50 = interaction.guild.roles.cache.find(
                (role) => role.name === 'Grandmaster (Vc)'
            )
            console.log(user2.level)
            if (user2.level > 1 && user2.level < 5) {
                if (
                    target.roles.cache.some((r) => r.name === role2.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
                        .setAuthor(
                            `${target.user.tag} hat eine neue Rolle bekommen!`,
            ...
            if (user2.level > 4 && user2.level < 10) {
                if (
                    target.roles.cache.some((r) => r.name === role5.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
            ...
            if (user2.level > 9 && user2.level < 20) {
                if (
                    target.roles.cache.some((r) => r.name === role10.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
            ...
            if (user2.level > 19 && user2.level < 30) {
                if (
                    target.roles.cache.some((r) => r.name === role20.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
            ...
            if (user2.level > 29 && user2.level < 40) {
                if (
                    target.roles.cache.some((r) => r.name === role30.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
            ...
            if (user2.level > 39 && user2.level < 50) {
                if (
                    target.roles.cache.some((r) => r.name === role40.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
            ...
            if (user2.level > 49) {
                if (
                    target.roles.cache.some((r) => r.name === role50.toString())
                ) {
                    return
                } else {
                    const embed = new MessageEmbed()
